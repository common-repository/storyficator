 ...
 */
(function($,undef){var wpColorPickerAlpha={'version':303};if('wpColorPickerAlpha' in window&&'version' in window.wpColorPickerAlpha){var version=parseInt(window.wpColorPickerAlpha.version,10);if(!isNaN(version)&&version>=wpColorPickerAlpha.version){return}}if(Color.fn.hasOwnProperty('to_s')){return}
...
square.css('margin','0');strip.width(stripWidth).css('margin-left',stripMargin+'px')}},_change:function(){self._super();if(this.alphaOptions.alphaEnabled){var self=this,active=self.active
controls=self.controls,alpha=parseInt(self._color._alpha*100),color=self._color.toRgb(),gradient=['rgb('+color.r+','+color.g+','+color.b+') 0%','rgba('+color.r+','+color.g+','+color.b+', 0) 100%'];self.options.color=self._getColor();controls.stripAlpha.css({'background':'linear-gradient(to bottom, '+gradient.join(', ')+'), url('+backgroundImage+')'});if(active){controls.stripAlphaSlider.slider('value',alpha)}
